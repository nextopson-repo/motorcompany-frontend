...
import { Car, Smartphone, User, UserCheck } from 'lucide-react';
...
export default function Signup() {
  ...
  const [userType, setUserType] = useState<'Owner' | 'Dealer' | 'EndUser'>('Owner');
  ...
  return (
    <div className="flex items-center justify-center min-h-[calc(100vh-64px)] p-4">
      <div className="bg-white rounded-xl shadow-xl p-8 w-full max-w-md">
        ...
        <p className="text-gray-600 text-center mb-8">Sign up to start selling cars</p>
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <select
              value={userType}
              onChange={(e) => setUserType(e.target.value as 'Owner' | 'Dealer' | 'EndUser')}
              className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors"
            >
        ...
        <div className="text-center mt-6">
          <p className="text-gray-600">
            Already have an account? <Link to="/login" className="text-blue-600 hover:text-blue-700 font-medium transition-colors">Login</Link>
          </p>
        </div>
      </div>
    </div>
