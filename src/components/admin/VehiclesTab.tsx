          ...
          <div key={vehicle.id} className="border border-gray-200 rounded-lg overflow-hidden hover:shadow-lg transition">
            <div className="relative">
              ...
              <div className="absolute top-3 right-3">
                <span className={`px-3 py-1 rounded-full text-xs font-semibold ${vehicle.condition === 'New' ? 'bg-green-500 text-white' : 'bg-orange-500 text-white'}`}>{vehicle.condition}</span>
              </div>
            </div>
            <div className="p-5">
              <h4 className="text-lg font-bold mb-2">{vehicle.title}</h4>
              <div className="text-2xl font-bold text-blue-600 mb-3">₹{vehicle.price.toLocaleString()}</div>
